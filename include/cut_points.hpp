...
void Dfs_Cut_Points(int* timer, std::unordered_set<size_t>* used, std::map<size_t, int>* entry_time, std::map<size_t, int>* min_of_time, std::vector<size_t>* points, T graph, size_t v, std::optional<size_t> opt) {
...
void CutPoints(T graph, std::vector<size_t>* points) {
  ...
  for (size_t v : graph.Vertices()) {
    if (used.find(v) == (used).end()) {
      ...
      Dfs_Cut_Points(&timer, &used, &entry_time, &min_of_time, points, graph, v, opt);
    }
  }
}
} // namespase graph
